...
export default function FooterSection() {
  return (
    <footer className="py-20 px-8 md:px-24 bg-white">
      {/* 1. FooterCTA Section (நீல நிற பாக்ஸ்) */}
      <div className="bg-indigo-600 rounded-[40px] overflow-hidden text-white mb-20">
        ...
        <div className="grid md:grid-cols-2 p-12 md:p-24 items-center gap-8">
          ...
          <div className="flex flex-col md:items-end justify-center gap-6">
            ...
            <button className="w-16 h-16 bg-white text-indigo-600 rounded-full flex items-center justify-center hover:bg-slate-100 transition">
              <ArrowRight size={28} />
            </button>
          </div>
        </div>
      ...
      {/* 2. Footer Links Section (கீழே உள்ள பகுதி) */}
      <div className="container mx-auto flex flex-col items-center text-center">
        ...
        <nav className="flex gap-8 mb-12">
          {['About', 'Work', 'Services', 'Jobs'].map((item) => (
            <a key={item} href={`#${item.toLowerCase()}`} className="text-sm font-bold text-slate-900 hover:text-indigo-600 transition">
              {item}
            </a>
          ))}
        </nav>
        <div className="w-full flex flex-col md:flex-row justify-between items-center text-xs text-slate-400 pt-8 border-t border-slate-100">
          <p>© 2020, UI8 LLC.</p>
